...
class Window(QMainWindow):
    ...
    def find_text(self):
        # first display the input dialogue and get the search text from it
        search_text, ok = QInputDialog.getText(self, "Find Text", "Search for")
        # if user clicks ok, begin the search
        if ok:
            # this list is used to store information about the selected text for highlighting
            all_words = []
            # set the cursor to the beginning of the edit field, this ensures the search
            ...
            while self.edit_field.find(search_text):
                # This creates instance of QTextEdit. Extra-selection which is used to define
                # the formatting and location of highlighted text
